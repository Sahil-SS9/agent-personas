...
The trial helper writes into a new destination and does not modify live profiles. The execution helper passes literal arguments without a shell, uses an environment allowlist and bounds time and output. It is separate from your platform's security controls: your agent CLI may still use its normal account and filesystem access. Use the CLI’s own permission and isolation controls.
